...
> セキュリティ注: アクセストークンは実行時にローカルの認証ファイルから読むだけで、**標準出力・ログ・リポジトリには一切出力しません**。認証ファイル自体は `.gitignore` 済みでコミットされません。
...
- 実行時に認証ファイル（`~/.claude/.credentials.json`）のアクセストークンを読み、`/api/oauth/usage` に問い合わせます。**トークンは標準出力・ログ・リポジトリに一切出力しません**。トークンの書き換えもしません。
- 解析するのは残量%とトークン数のみ。ログ本文（プロンプト内容）は扱いません。
- レートリミットの残量問い合わせであり、**Claude の利用枠（クレジット）は消費しません**。
- リポジトリには秘密情報を含めないこと。`~/.claude` はコミットしない（同梱の `.gitignore` が `.credentials.json` / `*.jsonl` 等を保険的に除外します）。
